...
class ManagerTeamTask(models.Model):
    _name = 'manager.team.task'
    _description = """ Manager Team Tasks"""
        ...
        for record in self:
            if not record.line_ids:
                ...
                last_line = self.env['customer.claim.line'].search([
                    ('manager_team_task_id', '=', record.id)], order="id desc",
                                                                   limit=1)
                if not last_line:
                    record.new_status = ""
                else:
    ...
    state = fields.Selection([
        ...
        default="receipted")
    line_ids = fields.One2many('customer.claim.line', 'manager_team_task_id',
                               'Lines')
